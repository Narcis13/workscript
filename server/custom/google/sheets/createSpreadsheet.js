...
let CreateSpreadsheetNode = class CreateSpreadsheetNode {
    metadata = {
    ...
    async execute(context) {
        ...
        try {
            ...
            if (config.sheets && Array.isArray(config.sheets)) {
                spreadsheetBody.sheets = config.sheets.map((sheetName, index) => ({
                    properties: {
                        sheetId: index,
                        title: sheetName,
                        index: index
...
CreateSpreadsheetNode = __decorate([
    RegisterNode
], CreateSpreadsheetNode);
export { CreateSpreadsheetNode };
