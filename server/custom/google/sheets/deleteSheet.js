...
let DeleteSheetNode = class DeleteSheetNode {
    metadata = {
        name: 'deleteSheet',
    ...
    async execute(context) {
        ...
        try {
            ...
            if (!sheetId && config.sheetTitle) {
                ...
                const sheetProperties = metadataResponse.data.sheets?.find(sheet => sheet.properties?.title === config.sheetTitle)?.properties;
                if (!sheetProperties) {
                    return {
                        not_found: () => ({
...
DeleteSheetNode = __decorate([
    RegisterNode
], DeleteSheetNode);
export { DeleteSheetNode };
