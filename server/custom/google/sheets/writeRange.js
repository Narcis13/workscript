...
let WriteRangeNode = class WriteRangeNode {
    metadata = {
        name: 'writeRange',
    ...
    async execute(context) {
        ...
        try {
            ...
            if (config.values.length > 0 && typeof config.values[0] === 'object' && !Array.isArray(config.values[0])) {
                ...
                values = config.values.map((obj) => keys.length > 0 ? keys.map(key => obj[key]) : Object.values(obj));
...
WriteRangeNode = __decorate([
    RegisterNode
], WriteRangeNode);
export { WriteRangeNode };
