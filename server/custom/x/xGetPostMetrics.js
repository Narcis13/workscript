...
let XGetPostMetricsNode = class XGetPostMetricsNode {
    metadata = {
    ...
    async execute({ config, state }) {
        ...
        try {
            ...
            for (const postId of metricsConfig.postIds) {
                const postData = responseData.data?.find((p) => p.id === postId);
                if (!postData) {
...
XGetPostMetricsNode = __decorate([
    RegisterNode
], XGetPostMetricsNode);
export { XGetPostMetricsNode };
