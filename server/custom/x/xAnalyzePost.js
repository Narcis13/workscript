...
let XAnalyzePostNode = class XAnalyzePostNode {
    metadata = {
        name: 'xAnalyzePost',
    ...
    async execute({ config, state }) {
        ...
        try {
            ...
            if (analyzeConfig.analysisTypes.includes('context') && post.context_annotations) {
                analysis.contextAnnotations = post.context_annotations.map((annotation) => ({
                    domain: {
    ...
    calculateSeverity(flags) {
        if (flags.includes('potential_violence'))
            return 'high';
        if (flags.includes('potential_spam'))
            return 'medium';
        return 'low';
    }
};
XAnalyzePostNode = __decorate([
    RegisterNode
], XAnalyzePostNode);
export { XAnalyzePostNode };
