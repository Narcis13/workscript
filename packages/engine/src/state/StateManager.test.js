...
import { StateManager, StateNotFoundError, StateLockError, SnapshotNotFoundError } from './StateManager';
...
describe('StateManager', () => {
    ...
    describe('initialization', () => {
        ...
        it('should throw error if state already exists for execution', async () => {
            await stateManager.initialize(testExecutionId, testInitialState);
            await expect(stateManager.initialize(testExecutionId, { other: 'data' })).rejects.toThrow(`State already exists for execution: ${testExecutionId}`);
        });
    ...
    describe('state retrieval', () => {
        ...
        it('should throw StateNotFoundError for non-existent execution', async () => {
            await expect(stateManager.getState('non-existent-id')).rejects.toThrow(StateNotFoundError);
        });
    ...
    describe('state updates', () => {
        ...
        it('should throw error for non-existent execution', async () => {
            await expect(stateManager.updateState('non-existent', { data: 'test' })).rejects.toThrow(StateNotFoundError);
        });
    });
    describe('state locking', () => {
        ...
        it('should prevent updates to locked keys', async () => {
            ...
            await expect(stateManager.updateState(testExecutionId, { counter: 10 })).rejects.toThrow(StateLockError);
            unlock();
        ...
        it('should prevent locking already locked keys', async () => {
            ...
            await expect(stateManager.lockKeys(testExecutionId, ['counter'])).rejects.toThrow(StateLockError);
        });
        it('should allow updates with lockKeys parameter', async () => {
            await stateManager.updateState(testExecutionId, { counter: 15, message: 'updated' }, ['counter', 'message']);
            const state = await stateManager.getState(testExecutionId);
    ...
    describe('snapshots', () => {
        ...
        it('should throw error for non-existent snapshot', async () => {
            await expect(stateManager.rollback(testExecutionId, 'non-existent-snapshot')).rejects.toThrow(SnapshotNotFoundError);
        });
    ...
    describe('persistence adapter integration', () => {
        ...
        it('should handle persistence adapter errors gracefully', async () => {
            ...
            await expect(stateManagerWithFailingAdapter.initialize(testExecutionId, testInitialState)).rejects.toThrow('Persistence failed');
            ...
            workingStateManager.persistenceAdapter = failingAdapter;
            await expect(workingStateManager.updateState('test-exec', { counter: 5 })).rejects.toThrow('Persistence failed');
        });
    ...
    describe('Nested Path Operations', () => {
        ...
        describe('setNestedPath', () => {
            ...
            it('should throw error for empty path', async () => {
                await expect(stateManager.setNestedPath(testExecutionId, '', 'value')).rejects.toThrow('Invalid state path: path cannot be empty');
            });
            it('should throw error for non-existent execution', async () => {
                await expect(stateManager.setNestedPath('non-existent', 'path.to.value', 'value')).rejects.toThrow(StateNotFoundError);
            });
        ...
        describe('getNestedPath', () => {
            ...
            it('should throw error for non-existent execution', async () => {
                await expect(stateManager.getNestedPath('non-existent', 'some.path')).rejects.toThrow(StateNotFoundError);
            });
        });
    });
    ...
    describe('Phase 3: State Change Detection', () => {
        ...
        describe('State Triggers', () => {
            it('should create conditional state triggers', async () => {
                let triggered = false;
                const triggerId = stateManager.createStateTrigger(testExecutionId, (changes) => changes.some(c => c.key === 'counter' && typeof c.newValue === 'number' && c.newValue >= 10), () => { triggered = true; });
                // Should not trigger
                await stateManager.updateState(testExecutionId, { counter: 5 });
            ...
            it('should support one-time triggers', async () => {
                let triggerCount = 0;
                const triggerId = stateManager.createStateTrigger(testExecutionId, (changes) => changes.some(c => c.key === 'counter'), () => { triggerCount++; }, { once: true });
                // First trigger
                await stateManager.updateState(testExecutionId, { counter: 5 });
            ...
            it('should support key-specific triggers', async () => {
                let triggered = false;
                const triggerId = stateManager.createStateTrigger(testExecutionId, (changes) => changes.some(c => c.key === 'counter'), () => { triggered = true; }, { keys: ['counter'] });
