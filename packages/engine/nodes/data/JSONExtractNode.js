...
export class JSONExtractNode extends WorkflowNode {
    ...
    async execute(context, config) {
        const { method = 'dot_notation', path, fallbackValue, multiple = false, outputField = 'extractedValue' } = config || {};
        // Validation
        if (!path) {
            return {
                error: () => ({
     ...
     */
    extractByBracketNotation(obj, path) {
        if (!path)
            return obj;
        let current = obj;
