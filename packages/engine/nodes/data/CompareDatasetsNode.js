...
export class CompareDatasetsNode extends WorkflowNode {
    ...
    async execute(context, config) {
        const { mode, input1, input2, input1Fields, input2Fields, matchFields, resolveConflicts, selectedBranch } = config || {};
        // Validate mode
        if (!mode) {
            return {
                error: () => ({
     ...
     */
    keepMatches(input1, input2, matchFields, input1Fields, input2Fields) {
        const result = [];
     ...
     */
    keepNonMatches(input1, input2, matchFields, input1Fields) {
        const result = [];
        const input2Map = this.buildHashMap(input2, matchFields.map(mf => mf.field2));
        for (const item1 of input1) {
            const key = this.buildKey(item1, matchFields.map(mf => mf.field1));
     ...
     */
    removeMatches(input1, input2, matchFields, input1Fields) {
     ...
     */
    enrich(input1, input2, matchFields, input2Fields, resolveConflicts = 'prefer_input1') {
        const result = [];
        const input2Map = this.buildHashMap(input2, matchFields.map(mf => mf.field2));
        for (const item1 of input1) {
            const key = this.buildKey(item1, matchFields.map(mf => mf.field1));
     ...
     */
    mergeItems(item1, item2, input1Fields, input2Fields) {
     ...
     */
    countMatches(input1, input2, matchFields) {
        const input2Map = this.buildHashMap(input2, matchFields.map(mf => mf.field2));
        let count = 0;
        for (const item1 of input1) {
            const key = this.buildKey(item1, matchFields.map(mf => mf.field1));
