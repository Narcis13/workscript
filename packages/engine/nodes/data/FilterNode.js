...
export class FilterNode extends WorkflowNode {
    ...
    async execute(context, config) {
        const { items, conditions, matchMode = 'all' } = config || {};
        // Validate inputs
     ...
     */
    evaluateConditions(item, conditions, matchMode) {
        if (matchMode === 'all') {
     ...
     */
    evaluateBooleanCondition(fieldValue, operation) {
        switch (operation) {
            case 'true':
     ...
     */
    evaluateObjectCondition(fieldValue, operation) {
        switch (operation) {
            case 'isEmpty':
                if (fieldValue === null || fieldValue === undefined) {
                    return true;
