...
export class MathOperationsNode extends WorkflowNode {
    ...
    async execute(context, config) {
        const { operation, mode = 'multiple_fields', field1, field2, value, fields, outputField = 'result', precision } = config || {};
        // Validate inputs
        if (!operation) {
            return {
                error: () => ({
     ...
     */
    async executeSingleFieldMode(context, operation, field1) {
        if (!field1) {
     ...
     */
    async executeMultipleFieldsMode(context, operation, field1, field2, value) {
        if (!field1) {
     ...
     */
    async executeAggregateMode(context, operation, fields) {
     ...
     */
    performUnaryOperation(operation, num) {
        switch (operation) {
            case 'sqrt':
                if (num < 0) {
     ...
     */
    performAggregateOperation(operation, values) {
        switch (operation) {
            case 'sum':
            case 'add':
