...
interface CalculateFieldNodeConfig {
...
export class CalculateFieldNode extends WorkflowNode {
  ...
  async execute(context: ExecutionContext, config?: unknown): Promise<EdgeMap> {
    const { calculations, onError = 'stop', defaultValue = null } = (config as CalculateFieldNodeConfig) || {};
   ...
   */
  private resolveFieldReferences(formula: string, data: Record<string, any>): string {
