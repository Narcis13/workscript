...
must_haves:
  ...
  artifacts:
    ...
    - path: "apps/api/src/plugins/workscript/services/WorkflowService.ts"
      provides: "Service injection for workflow context"
      contains: "flexRecord"
  key_links:
    - from: "FlexRecordNode"
      to: "context.services.flexRecord"
      via: "service injection in WorkflowService"
      pattern: "context.*services.*flexRecord"
    - from: "WorkflowService"
      to: "flexRecordService"
      via: "service registration"
      pattern: "services.*flexRecord.*=.*flexRecordService"
    - from: "FlexRecordNode"
      to: "context.state"
...
<objective>
Create the FlexRecordNode workflow node for record CRUD operations and add relationship validation with include support to FlexRecordService.

Purpose: Enable workflows to perform complete record operations (insert, find, update, delete) with relationship handling following the established FlexTableNode pattern.

Output:
- FlexRecordNode workflow node registered in @workscript/nodes
- Service injection registered in WorkflowService
...
<task type="auto">
  <name>Task 1: Create FlexRecordNode workflow node with service registration</name>
  <files>
    packages/nodes/src/flexdb/FlexRecordNode.ts
    packages/nodes/src/index.ts
    apps/api/src/plugins/workscript/services/WorkflowService.ts
  ...
  <action>
**Part A: Create FlexRecordNode.ts following the FlexTableNode pattern exactly:**

1. Add JSDoc header documenting:
   - Service injection pattern (context.services.flexRecord)
...
4. Implement execute(context, config):
   - Get flexRecordService from context.services.flexRecord
   - Return error edge if service not available
   - Resolve tableId from config or context.state.flexTableId
   - Switch on operation: insert, findOne, findMany, update, delete
   - Wrap in try/catch for internal errors
...
**Part B: Register FlexRecordService in WorkflowService for context injection:**

The FlexTableNode uses `context.services.flexDB` - we need to add `context.services.flexRecord` similarly.

1. In WorkflowService.ts, add import at top:
   ```typescript
   import { flexDBService, flexRecordService } from './services/FlexRecordService';
   ```

2. Modify the executeWorkflow method to inject services into the execution context:
   ```typescript
   public async executeWorkflow(workflowDefinition: WorkflowDefinition, initialState?: any) {
     ...
     // Inject services into execution context
     // The engine passes this context to node execute() methods
     ...
     // Execute workflow with services context
     return await this.executionEngine.execute(parsedWorkflow, executionContext);
   }
   ```

   **Note:** Check how ExecutionEngine.execute accepts additional context. If it doesn't support service injection directly, you may need to:
   - Extend the parsed workflow with a services property
   - Or modify how the engine creates ExecutionContext for nodes

   Look at how FlexTableNode accesses `context.services.flexDB` and ensure the same pattern works for `context.services.flexRecord`.
  </action>
  <verify>
...
- Check that FlexTableNode's pattern for accessing context.services.flexDB is mirrored
- Ensure flexRecordService is imported and registered
  </verify>
  <done>
...
- Service injection via context.services.flexRecord
- Registered in ALL_NODES in packages/nodes/src/index.ts

WorkflowService.ts updated with:
- flexRecordService import
- Service registration so context.services.flexRecord is available to nodes
  </done>
</task>
...
4. Service injection verification:
   ```bash
   grep -n "flexRecord" apps/api/src/plugins/workscript/services/WorkflowService.ts
   ```
   Should show service registration
...
- [ ] Node follows FlexTableNode pattern (service injection, edge routing, state updates)
- [ ] flexRecordService registered in WorkflowService for context.services.flexRecord
