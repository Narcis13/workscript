...
<task type="auto">
  <name>Task 1: Create FlexRecordNode workflow node</name>
  <files>
    packages/nodes/src/flexdb/FlexRecordNode.ts
    packages/nodes/src/index.ts
  </files>
  <action>
Create FlexRecordNode.ts following the FlexTableNode pattern exactly:
...
4. Workflow integration test (optional):
