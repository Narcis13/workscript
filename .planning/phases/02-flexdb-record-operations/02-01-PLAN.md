...
must_haves:
  truths:
    - "System can insert records with JSON data into flex_records table"
    - "System extracts indexed values to idx_str_1-3, idx_num_1-2, idx_date_1-2 on insert"
    - "System populates search_text column from string/text fields"
    - "System can find single record by ID"
    - "System can find many records with filters and pagination"
    - "System can update records with version increment (optimistic locking)"
    - "System can soft-delete or hard-delete records"
  artifacts:
...
3. Implement buildSingleCondition(condition, schema, indexedColumns):
   ...
   - For 'search' operator: Use MATCH AGAINST on searchText column

4. Implement buildFilterConditions(filter, schema, indexedColumns):
   - Handle simple equality filter: { field: value } -> eq condition
   - Handle advanced filters with AND/OR/NOT
   - Return SQL condition or undefined if no filters
...
- Combine conditions with and() / or() / not() from drizzle-orm
  ...
  <verify>
Check: `cd /Users/narcisbrindusescu/teste/workscript/apps/api && bun run typecheck`
...
- Support for AND/OR/NOT logic operators
  </done>
</task>
