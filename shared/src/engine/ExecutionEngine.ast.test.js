...
describe('ExecutionEngine AST Execution', () => {
    ...
    describe('Deeply Nested Workflow Execution', () => {
        it('should execute a complex deeply nested workflow correctly', async () => {
            ...
            expect(trace.some((entry) => entry.nodeId === 'decision' && entry.depth === 0)).toBe(true);
            // Should have executed nested nodes
            expect(trace.some((entry) => entry.depth > 0)).toBe(true);
        });
        it('should execute array sequences with nested nodes correctly', async () => {
            ...
            expect(trace.some((entry) => entry.message === 'Start pipeline')).toBe(true);
        ...
        it('should handle parent-child execution context correctly', async () => {
            ...
            expect(trace.some((entry) => entry.message === 'Root parent' && entry.depth === 0)).toBe(true);
            expect(trace.some((entry) => entry.nodeId === 'decision' && entry.depth === 1)).toBe(true);
            expect(trace.some((entry) => entry.message === 'Deep child' && entry.depth === 2)).toBe(true);
        });
        it('should execute multiple nested branches correctly', async () => {
            ...
            expect(trace.some((entry) => entry.message === 'Left branch')).toBe(true);
        });
    });
