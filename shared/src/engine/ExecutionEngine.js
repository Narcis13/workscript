...
export class ExecutionEngineError extends Error {
    ...
    nodeId;
    originalError;
    constructor(message, executionId, nodeId, originalError) {
        super(message);
...
export class LoopLimitError extends ExecutionEngineError {
    constructor(executionId, nodeId) {
...
export class ExecutionEngine {
    ...
    stateManager;
    hookManager;
    static MAX_LOOP_ITERATIONS = 1000;
    static DEFAULT_TIMEOUT = 30000; // 30 seconds
    stateResolver;
    constructor(registry, stateManager, hookManager = new HookManager()) {
     ...
     */
    createInitialContext(workflow, executionId) {
        return {
            state: {},
            inputs: {},
            workflowId: workflow.id,
     ...
     */
    async executeWorkflowSequence(workflow, context) {
        let currentIndex = 0;
        const loopCounts = new Map();
        while (currentIndex < workflow.nodes.length) {
            const node = workflow.nodes[currentIndex];
            if (!node) {
                throw new ExecutionEngineError(`Node not found at index ${currentIndex}`, context.executionId);
            }
    ...
    async executeNode(node, context) {
        try {
            ...
            throw new ExecutionEngineError(`Node execution failed: ${node.nodeId} - ${error instanceof Error ? error.message : String(error)}`, context.executionId, node.nodeId, error instanceof Error ? error : new Error(String(error)));
        }
    }
    /**
     ...
     */
    async processEdgeMap(edgeMap, context) {
     ...
     * Requirement 10.2: WHEN a node completes THEN it must route based on the returned edge
     */
    async resolveEdgeRoute(result, node, workflow, currentIndex, context) {
        const isLoopNode = node.nodeId.endsWith('...');
     ...
     */
    async executeNodeFromRegistry(nodeId, context) {
     ...
     */
    async executeSequenceFromParsedEdge(sequence, context) {
     ...
     */
    async executeNestedNode(parsedNode, context) {
        try {
     ...
     */
    async executeSequence(sequence, context) {
        for (const item of sequence) {
            if (typeof item === 'string') {
                // Execute referenced node using the centralized method
                await this.executeNodeFromRegistry(item, context);
     ...
     */
    async executeNestedConfiguration(config, context) {
        for (const [nodeId, nodeConfig] of Object.entries(config)) {
            ...
            await instance.execute(nodeContext, resolvedConfig);
            await this.updateStateFromContext(context.executionId, nodeContext);
        }
    }
    /**
     ...
     */
    async prepareNodeInputs(executionId, node) {
     ...
     */
    async updateStateFromContext(executionId, context) {
     ...
     */
    async executeHooks(eventType, contextData) {
        const hookContext = {
            eventType,
            timestamp: new Date(),
            ...contextData
     ...
     */
    calculateStateChanges(stateBefore, stateAfter) {
        const changes = {};
