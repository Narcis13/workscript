...
export class ExtractTextNode extends WorkflowNode {
    ...
    async execute(context, config) {
        const { method, field, pattern, flags, startDelimiter, endDelimiter, extractType = 'custom', occurrence = 0, outputField = 'extractedText' } = config || {};
        // Validate method
        if (!method) {
            return {
                error: () => ({
     ...
     */
    extractWithRegex(text, pattern, flags) {
        try {
     ...
     */
    extractBetween(text, startDelimiter, endDelimiter) {
        const matches = [];
     ...
     */
    extractAll(text, extractType, customPattern, flags) {
        let regex;
     ...
     */
    extractSpecific(text, extractType, occurrence, customPattern, flags) {
        if (occurrence < 0) {
