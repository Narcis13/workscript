...
export class RemoveDuplicatesNode extends WorkflowNode {
    ...
    async execute(context, config) {
        const { operation = 'current_input', compareMode = 'all_fields', fieldsToCompare = [], fieldsToExclude = [], removeOtherFields = false, disableDotNotation = false, scope = 'node', historySize = 1000 } = config || {};
        try {
     ...
     */
    getItemHash(item, compareMode, fieldsToCompare, fieldsToExclude, disableDotNotation) {
     ...
     */
    filterFields(item, compareMode, fieldsToCompare, fieldsToExclude) {
        if (!item || typeof item !== 'object') {
            return item;
        }
        const filtered = {};
