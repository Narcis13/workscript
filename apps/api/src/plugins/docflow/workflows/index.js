...
async function findWorkflowById(workflowId) {
    const workflowsDir = join(process.cwd(), 'workflows');
    try {
...
async function searchWorkflowRecursively(dir, workflowId, baseDir) {
    if (!baseDir)
        baseDir = dir;
    try {
            ...
            if (entry.isDirectory()) {
                ...
                if (fileBaseName === workflowId || entry.name === `${workflowId}.json`) {
                    try {
                        ...
                        const workflowDefinition = JSON.parse(fileContent);
...
async function getAllWorkflows(dir) {
    const workflows = [];
    try {
        const entries = await readdir(dir, { withFileTypes: true });
        for (const entry of entries) {
